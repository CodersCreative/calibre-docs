...
- `&T` and `&mut T` are ordinary type-level references used in normal Calibre code
- expression-level reference and dereference use postfix syntax such as `value.&`, `value.&mut`, and `value.*`
