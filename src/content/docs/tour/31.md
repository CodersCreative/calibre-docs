...
impl CountryBase {
  const get_language := fn (self : &CountryBase) -> Language => return self.language;
};
...
```cal
impl int {
  const days := fn (self : Self) -> Self => return self * 24 * 60 * 60;
};
...
impl CountryBase {
  ...
  const english := fn -> CountryBase => return CountryBase { language : Self.default_language };
};
```
