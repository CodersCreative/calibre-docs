...
```cal
let classified := match 30 {
  in [30, 40, 50] => emit "in-list",
  _ => emit "other"
};
```
...
```cal
let classify := fn match int -> str {
  v @ is int if v > 100 => return "large-int",
  _ => return "other"
};
```
...
```cal
let classify := fn match int -> str {
  in 0..20 if true => return "small-range",
  let v if v > 100 => return "large-int",
  _ => return "other"
};
```
...
let classify_list := fn match list:<int> -> str {
  [] => return "Empty list",
  [1] => return "List of just 1",
  [4, ..] => return "List starting with 4",
  [_, _] => return "List of 2 elements",
  _ => return "Some other list"
};
```
...
let parse_command := fn match str -> str {
  "go:" & direction => return "move " & direction,
  head @ "noop" & _ => return "saw " & head,
  _ => return "unknown"
};
```
